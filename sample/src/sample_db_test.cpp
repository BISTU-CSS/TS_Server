...
UNUSED static ASN1_INTEGER *xSerialCb(UNUSED struct TS_resp_ctx *ctx,
                                      UNUSED void *r) {
...
int main(int argc, UNUSED char *argv[]) {
  ...
  //  *>(add.data()); d2i_TS_REQ(&ts_req, &t, result_length); std::cout <<
  //  TS_REQ_get_version(ts_req) << std::endl; TS_MSG_IMPRINT *msg_imprint =
  //  TS_REQ_get_msg_imprint(ts_req);
  //
  ...
  std::string base64_request =
      "MEsCAQEwLzALBglghkgBZQMEAgEEIOd5Oc5rtdlXOFEtG2mNTsxntaw1TqHttt2Xf+"
      "EG3FFZBgcqAwQFBgcIAgkAxR4SDwQ5x+oBAf8=";
  char request[77];
  Base64::Decode(base64_request.c_str(), base64_request.length(), request, 77);
  std::string tsa_cert_pem =
      "-----BEGIN CERTIFICATE-----\n"
  ...
  std::string pri_key =
      ...
      "-----END RSA PRIVATE KEY-----";
  BIO *pri_key_bio =
      BIO_new_mem_buf(reinterpret_cast<const unsigned char *>(pri_key.c_str()),
                      pri_key.length());
  RSA *rsa_key =
  ...
  ts_resp = TS_RESP_create_response(ts_resp_ctx, req_bio);
  {
    ...
    std::cout << ASN1_INTEGER_get(TS_STATUS_INFO_get0_status(
                     TS_RESP_get_status_info(ts_resp)))
              << std::endl;
    std::cout << ASN1_INTEGER_get(TS_STATUS_INFO_get0_failure_info(
                     TS_RESP_get_status_info(ts_resp)))
              << std::endl;
    std::cout << "DSDS" << std::endl;

    for (int i = 0; i < 10; i++) {
      std::cout << ASN1_BIT_STRING_get_bit(
